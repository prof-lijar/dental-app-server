...
// GET /api/child-activity/latest-health-report?childId=xxx
export async function GET(request: NextRequest) {
    try {
        ...
        // Get child latest health report
        const result = await DentalHealthCheckService.getChildLatestHealthReport(
            authUser.userId,
            childId
        );
        ...
        if (!result) {
            return NextResponse.json(
                { error: "No health check report found for this child" },
                { status: 404 }
            );
        }
        ...
        console.error("Child latest health report error:", error);
